...
def _make_base_prompt(question: str) -> str:
    """
    Raw question passed directly to the base model — no system prompt,
    no format instructions.  The model generates whatever it naturally
    would without any guidance, making the contrast with the RL model
...
def _infer(
    ...
    if use_chat_template:
        # Fine-tuned model: full structured prompt (system + user via chat template)
        ...
        try:
            ...
            prompt = _make_base_prompt(question)
            use_chat_template = False
    else:
        # Base model: just the raw question, no instructions whatsoever
        prompt = _make_base_prompt(question)
...
def _write_markdown(out_dir: Path, summ_a: Dict, summ_b: Dict, meta: Dict) -> Path:
    ...
    lines = [
        ...
        f"**Base prompt:** raw question only (no instructions)  ",
        f"**RL prompt:** full system + step-format instructions (`create_solver_messages`)  ",
        "",
...
<div class="meta">
  ...
  <div><span>Base model</span><br><strong>{base_model_name}</strong></div>
  <div><span>Base prompt</span><br><strong style="color:#dc2626">Raw question only</strong></div>
  <div><span>RL model</span><br><strong>{rl_model_name}</strong></div>
  <div><span>RL prompt</span><br><strong style="color:#16a34a">Full system + format prompt</strong></div>
</div>
...
<p style="font-size:.75rem;color:#94a3b8">
  ...
  Base model prompt: <em>raw question only</em> ·
  RL model prompt: <code>create_solver_messages()</code> (system + step format) ·
  Step scoring: reference-solution alignment via <code>_score_steps()</code>
</p>
</body>
</html>
"""
...
<div class="q-card {oc}">
  ...
  <div class="q-body">
    <div class="q-col">
      ...
      <div style="font-size:.7rem;color:#94a3b8;margin-bottom:.35rem">prompt: raw question only</div>
      {_steps_badge(a)}
      {_render_steps(a, "model_a")}
    </div>
    <div class="q-col">
      <div class="col-title rl-title">{_esc(summ_b["label"])}</div>
      <div style="font-size:.7rem;color:#94a3b8;margin-bottom:.35rem">prompt: system + step format</div>
      {_steps_badge(b)}
